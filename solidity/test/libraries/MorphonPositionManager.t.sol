...
contract MorphonPositionManagerTest is Test {
    ...
    function setUp() public {
        ...
        MorphonPositionManager.MorphonPositionManagerConfig memory config = MorphonPositionManager
            .MorphonPositionManagerConfig({
    ...
    function test_Supply_WithSpecificAmount() public {
        ...
        // Then
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance - SUPPLY_AMOUNT,
            "Input account balance should decrease"
        );
        assertEq(
            mockLendingPool.balanceOf(address(mockAsset), address(outputAccount)),
            initialOutputMTokenBalance + SUPPLY_AMOUNT,
            "Output account mToken balance should increase"
        );
    }
    ...
    function test_Supply_WithZeroAmount() public {
        ...
        assertEq(
            mockLendingPool.balanceOf(address(mockAsset), address(outputAccount)),
            initialOutputMTokenBalance + initialInputBalance,
            "Output account mToken balance should increase by full amount"
        );
    }
    ...
    function test_Supply_WithInsufficientBalance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "ERC20InsufficientBalance(address,uint256,uint256)", address(inputAccount), INITIAL_BALANCE, largeAmount
            )
        );
    ...
    function test_Withdraw_WithSpecificAmount() public {
        ...
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance + withdrawAmount,
            "Input account balance should increase"
        );
        assertEq(
            mockLendingPool.balanceOf(address(mockAsset), address(outputAccount)),
            initialOutputMTokenBalance - withdrawAmount,
            "Output account mToken balance should decrease"
        );
    }
    ...
    function test_Withdraw_WithZeroAmount() public {
        ...
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance + initialOutputMTokenBalance,
            "Input account balance should increase by full amount"
        );
        assertEq(
            mockLendingPool.balanceOf(address(mockAsset), address(outputAccount)),
            0,
            "Output account mToken balance should be zero"
        );
    }
    ...
    function test_Borrow_WithStableInterestRate() public {
        ...
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance + BORROW_AMOUNT,
            "Input account balance should increase"
        );
        assertEq(
            mockLendingPool.borrowBalanceOf(address(mockAsset), address(inputAccount)),
            initialBorrowBalance + BORROW_AMOUNT,
            "Borrow balance should increase"
        );
    }
    ...
    function test_Repay_WithSpecificAmountAndStableRate() public {
        ...
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance - repayAmount,
            "Input account balance should decrease"
        );
        assertEq(
            mockLendingPool.borrowBalanceOf(address(mockAsset), address(inputAccount)),
            initialBorrowBalance - repayAmount,
            "Borrow balance should decrease"
        );
    }
    ...
    function test_Repay_WithZeroAmount() public {
        ...
        assertEq(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance - initialBorrowBalance,
            "Input account balance should decrease by borrow amount"
        );
        assertEq(
            mockLendingPool.borrowBalanceOf(address(mockAsset), address(inputAccount)),
            0,
    ...
    function test_UpdateConfig() public {
        // Given
        MorphonPositionManager.MorphonPositionManagerConfig memory newConfig = MorphonPositionManager
            .MorphonPositionManagerConfig({
            inputAccount: inputAccount,
            outputAccount: outputAccount,
            lendingPoolAddress: address(mockLendingPool),
        ...
        // Then
        (
            BaseAccount inputAccountConfig,
            BaseAccount outputAccountConfig,
    ...
    function test_CompleteLendingCycle() public {
        ...
        assertGt(
            mockAsset.balanceOf(address(inputAccount)),
            initialInputBalance - SUPPLY_AMOUNT,
            "Input account should have more than initial - supply"
        );
        assertGt(
            mockLendingPool.balanceOf(address(mockAsset), address(outputAccount)),
            0,
            "Output account should have remaining mTokens"
        );
        assertGt(
            mockLendingPool.borrowBalanceOf(address(mockAsset), address(inputAccount)),
            0,
            "Input account should have remaining debt"
        );
    }
    ...
    function test_MultipleOperationsWithDifferentRates() public {
        ...
        // Then
        assertEq(
            mockLendingPool.borrowBalanceOf(address(mockAsset), address(inputAccount)), 0, "All debt should be repaid"
        );
    }
}
